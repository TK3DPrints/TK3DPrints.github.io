...
document.addEventListener('tk:ready', () => {
  ...
  document.getElementById('limits-text').textContent = L.maxFileMB + ' MB per file and ' + L.maxTotalMB + ' MB in total, maximum ' + L.maxFiles + ' files';

  // Fill the material dropdown and color suggestions from business.js
  const material = form.elements.namedItem('material');
  BUSINESS.materials.forEach(m => material.append(h('option', { value: m, text: m })));
  const colorList = document.getElementById('color-suggestions');
  Object.keys(BUSINESS.colorSwatches || {}).forEach(c => colorList.append(h('option', { value: c })));
  ...
  function renderList(kind) {
    ...
    pick.list.replaceChildren(...pick.files.map(file => {
      const thumb = kind === 'image'
        ? h('img', { class: 'thumb', src: URL.createObjectURL(file), alt: '', width: 48, height: 48 })
        : h('span', { class: 'thumb thumb-file', 'aria-hidden': 'true', text: Forms.extOf(file.name).toUpperCase() });
      return h('li', {},
        thumb,
        h('span', { class: 'file-name', text: file.name }),
        h('span', { class: 'file-size', text: kb(file.size) }),
        h('button', { type: 'button', class: 'link-button', 'aria-label': 'Remove ' + file.name, text: 'Remove',
          onclick: () => { pick.files = pick.files.filter(f => f !== file); Forms.clearError(pick.input); renderList(kind); } }));
    }));
  }
  ...
  Object.entries(picks).forEach(([kind, pick]) => {
    pick.input.addEventListener('change', () => {
      ...
      if (overall) {                                   // undo the files that pushed us over the limit
        problems.push(overall);
        pick.files = pick.files.filter(f => !Array.from(pick.input.files).includes(f));
      }
      pick.input.value = '';                           // lets the same file be chosen again later
      renderList(kind);
  ...
  Forms.wire(form, {
    ...
    collect: () => {
      const v = n => form.elements.namedItem(n).value.trim();   // namedItem: 'length' would otherwise clash with elements.length
    ...
    onSuccess: f => {
      form.replaceWith(h('div', { class: 'success', id: 'quote-success', role: 'status' },
        h('h2', { text: 'Request received' }),
        h('p', { text: 'Thanks, ' + f['Name'] + '. We will review your request and email ' + f['Email'] + ' with questions or a quote. Submitting this form does not guarantee a price.' }),
        h('a', { class: 'btn btn-ghost', href: 'index.html', text: 'Back to home' })));
